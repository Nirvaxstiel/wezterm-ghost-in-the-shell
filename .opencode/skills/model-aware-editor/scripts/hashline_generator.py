...
def find_edit_location(
    ...
    if target_line:
        for line in lines:
            ...
            if (
                match
                and match.group(1) == target_line
                and match.group(2) == target_code
            ):
                return {
                    "type": "single",
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    if is_valid:
        print("[OK] All hashlines valid")
        return 0
    else:
        print(f"[FAIL] {len(issues)} hashline mismatches:")
        for issue in issues[:10]:
            print(
                f"  Line {issue['line']}: expected {issue['expected']}, got {issue['actual']}"
            )
...
def cmd_find(args: argparse.Namespace) -> int:
    ...
    result = find_edit_location(content, args.hash, args.range_start, args.range_end)
...
def main():
    ...
    p_gen = subparsers.add_parser(
        "generate", help="Generate hashlines (alias for read)"
    )
