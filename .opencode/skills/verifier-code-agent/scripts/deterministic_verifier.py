...
def check_imports_python(file_path: str) -> VerificationResult:
    ...
    try:
        ...
        stdlib = {
            ...
            'itertools', 'functools', 'operator', 'string', 'struct',
            'math', 'random', 'statistics', 'urllib', 'http', 'email',
            'html', 'xml', 'csv', 'io', 'pickle', 'sqlite3'
        }
        ...
        if missing:
            result.add_check("imports_python", False, f"Missing imports: {', '.join(missing)}")
        else:
    ...
    for test_file in test_files[:3]:  # Limit to 3 test files
        try:
            ...
            if proc.returncode == 0:
                ...
                if failures:
                    result.add_check(f"tests_{test_file.name}", False, f"Failures: {', '.join(failures[:3])}")
                else:
                    result.add_check(f"tests_{test_file.name}", False, f"Tests failed (exit {proc.returncode})")
            ...
            result.add_check(f"tests_{test_file.name}", False, f"Check failed: {str(e)}")
...
def check_security(file_path: str) -> VerificationResult:
    ...
    if re.search(r'execute\s*\(\s*f["\']', content) or re.search(r'execute\s*\(\s*["\'].*%s', content):
...
def check_dependencies(file_path: str) -> VerificationResult:
    ...
    try:
        if dep_file.name == "requirements.txt":
            proc = subprocess.run(
                [sys.executable, "-m", "pip", "install", "-r", str(dep_file), "--dry-run"],
                capture_output=True,
                text=True,
                timeout=30
            )
...
def main():
    ...
    parser.add_argument("--check", action="append", help="Check type (syntax, imports, tests, lint, security, dependencies, all)")
    ...
    if args.json:
        ...
        if not result.passed:
            print("\n⚠️  Some checks failed - consider review before proceeding")
            return 1
